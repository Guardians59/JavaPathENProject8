...
public class UserRewardControllerTest {
	...
	mockMvc.perform(
		...
		.andExpect(jsonPath("$.internalUser10").exists())
    ...
    @Test
    public void calculAllRewardTest() throws Exception {
	//GIVEN
	User user = userRepository.getUser("internalUser30");
	Double latitude = user.getListVisitedLocations().get(0).location.latitude;
	Double longitude = user.getListVisitedLocations().get(0).location.longitude;
	User user2 = userRepository.getUser("internalUser40");
	...
	when(microServiceRewardCentralProxyMock.getRewardPoints(mapId)).thenReturn(rewardDisney);
	when(microServiceRewardCentralProxyMock.getRewardPoints(mapIdSecond)).thenReturn(rewardJackson);
	when(microServiceRewardCentralProxyMock.getRewardPoints(mapIdTree)).thenReturn(rewardMojave);
	//THEN
	mockMvc.perform(MockMvcRequestBuilders.get("/calculateAllRewardsPointOfUsers")
			.contentType(MediaType.APPLICATION_JSON)
			.accept(MediaType.APPLICATION_JSON))
			.andExpect(jsonPath("$.internalUser30").exists())
			.andExpect(jsonPath("$.internalUser40").exists())
			.andExpect(status().isOk())
			.andDo(MockMvcResultHandlers.print());
    }
    
    @Test
    public void calculAllRewardErrorTest() throws Exception {
	//GIVEN
	User user = userRepository.getUser("internalUser80");
	List<Attraction> list = new ArrayList<>();
	UUID idDisney = UUID.randomUUID();
	UUID idJackson = UUID.randomUUID();
	HashMap<String, Object> mapId = new HashMap<>();
	...
	mockMvc.perform(MockMvcRequestBuilders.get("/calculateAllRewardsPointOfUsers")
			.contentType(MediaType.APPLICATION_JSON)
			.accept(MediaType.APPLICATION_JSON))
			.andExpect(jsonPath("$.internalUser80").doesNotExist())
			.andExpect(status().isNotFound());
    }
